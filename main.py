...
def main():
    ...
    if args.buy:
        csv_writer(buy_csv, args.prod, args.amount, args.price, args.exp)
...
def csv_writer(csv_file, prod, amnt, price, exp):
    ## If file doesn't exist, create it with the correct headers
    if not os.path.exists(csv_file):
        file = Path(csv_file)
        file.touch()
        # Write header
        headerlist = ['ID', 'Product', 'Amount', 'BuyPrice', 'Exp-Date']
    ...
    with open (csv_file, 'a', newline='') as open_csv:
        ...
        new_row = [int(last_used_ID)+1, prod, amnt, price, exp]
